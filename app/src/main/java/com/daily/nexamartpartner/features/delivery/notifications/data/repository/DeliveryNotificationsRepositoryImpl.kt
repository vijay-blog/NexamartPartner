...
class DeliveryNotificationsRepositoryImpl(private val source:DeliveryNotificationsDataSource):DeliveryNotificationsRepository{
 override suspend fun list(q:DeliveryNotificationQuery):AppResult<DeliveryNotificationPage>{return when(val r=source.list(q)){is AppResult.Success->{val d=r.data;AppResult.Success(DeliveryNotificationPage((d.content?:emptyList()).map{DeliveryNotification(it.id.orEmpty(),it.title.orEmpty(),it.message.orEmpty(),it.createdAt,it.read?:false,it.type,it.orderId?.toString(),it.actionUrl)} ,d.page?:d.number?:q.page,d.pageSize?:d.size?:q.pageSize,d.totalPages?:0,d.totalElements?:0L,d.hasNextPage?:false,d.unreadCount))};is AppResult.Failure->r}}
